...
class GetExpenseCategoriesUseCase implements GetListUseCase<ExpenseCategory> {
  ...
  @override
  Future<Result<List<ExpenseCategory>>> call() =>
...
class GetExpenseCategoryUseCase implements GetByIdUseCase<ExpenseCategory> {
  ...
  final ExpenseCategoryRepository _repository;
  @override
  Future<Result<ExpenseCategory>> call(String params) =>
      _repository.getExpenseCategoryById(params);
}
...
class SearchExpenseCategoriesUseCase implements SearchUseCase<ExpenseCategory> {
  ...
  final ExpenseCategoryRepository _repository;
  @override
  Future<Result<List<ExpenseCategory>>> call(String params) =>
      _repository.searchExpenseCategories(params);
}

class CreateExpenseCategoryUseCase
    implements CreateUseCase<ExpenseCategory, ExpenseCategoryInput> {
  ...
  @override
  Future<Result<ExpenseCategory>> call(ExpenseCategoryInput params) async {
    final validation = _validationService.validateForSave(params);
    return validation.when(
      success: _repository.createExpenseCategory,
      failure: (f) async => Failure<ExpenseCategory>(f),
    );
  }
}

class UpdateExpenseCategoryUseCase
    implements UpdateUseCase<ExpenseCategory, ExpenseCategoryInput> {
  const UpdateExpenseCategoryUseCase(this._repository, this._validationService);
  final ExpenseCategoryRepository _repository;
  final ExpenseCategoryValidationService _validationService;
  @override
  Future<Result<ExpenseCategory>> call(
    UpdateParams<ExpenseCategoryInput> params,
  ) async {
    final validation = _validationService.validateForSave(params.input);
    return validation.when(
      success: (i) => _repository.updateExpenseCategory(params.id, i),
      failure: (f) async => Failure<ExpenseCategory>(f),
    );
  }
}
...
class DeleteExpenseCategoryUseCase implements DeleteUseCase {
  ...
  Future<Result<void>> call(String params) =>
      _repository.softDeleteExpenseCategory(params);
}
