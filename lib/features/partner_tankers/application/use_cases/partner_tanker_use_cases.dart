...
class GetPartnerTankersUseCase implements GetListUseCase<PartnerTanker> {
  ...
  @override
  Future<Result<List<PartnerTanker>>> call() => _repository.getPartnerTankers();
}
...
class GetPartnerTankerUseCase implements GetByIdUseCase<PartnerTanker> {
  ...
  final PartnerTankerRepository _repository;
  @override
  Future<Result<PartnerTanker>> call(String params) =>
      _repository.getPartnerTankerById(params);
}
...
class SearchPartnerTankersUseCase implements SearchUseCase<PartnerTanker> {
  ...
  final PartnerTankerRepository _repository;
  @override
  Future<Result<List<PartnerTanker>>> call(String params) =>
      _repository.searchPartnerTankers(params);
}

class CreatePartnerTankerUseCase
    implements CreateUseCase<PartnerTanker, PartnerTankerInput> {
  ...
  @override
  Future<Result<PartnerTanker>> call(PartnerTankerInput params) async {
    final validation = _validationService.validateForSave(params);
    return validation.when(
      success: _repository.createPartnerTanker,
      failure: (f) async => Failure<PartnerTanker>(f),
    );
  }
}

class UpdatePartnerTankerUseCase
    implements UpdateUseCase<PartnerTanker, PartnerTankerInput> {
  const UpdatePartnerTankerUseCase(this._repository, this._validationService);
  final PartnerTankerRepository _repository;
  final PartnerTankerValidationService _validationService;
  @override
  Future<Result<PartnerTanker>> call(
    UpdateParams<PartnerTankerInput> params,
  ) async {
    final validation = _validationService.validateForSave(params.input);
    return validation.when(
      success: (i) => _repository.updatePartnerTanker(params.id, i),
      failure: (f) async => Failure<PartnerTanker>(f),
    );
  }
}
...
class DeletePartnerTankerUseCase implements DeleteUseCase {
  ...
  Future<Result<void>> call(String params) =>
      _repository.softDeletePartnerTanker(params);
}
