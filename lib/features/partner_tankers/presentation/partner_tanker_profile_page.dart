    ...
    return AppScreen(
      children: [
        PageHeader(
          ...
          subtitle: 'Partner Tanker Profile',
          trailing: IconButton.filledTonal(
            onPressed:
                () => ref
                    .read(navigationServiceProvider)
                    .goToEditPartnerTanker(context, partnerTanker.id),
        ...
        GlassCard(
          child: Column(
            children: [
              ...
              _DetailRow(
                label: 'Registration',
                value: partnerTanker.registrationNumber,
              ),
              const SizedBox(height: AppSpacing.sm),
              _DetailRow(
                label: 'Phone',
                value: partnerTanker.phone?.value ?? 'Not set',
              ),
              const SizedBox(height: AppSpacing.sm),
              _DetailRow(
                label: 'Notes',
                value: partnerTanker.notes ?? 'Not set',
              ),
              const SizedBox(height: AppSpacing.sm),
              _DetailRow(
                label: 'Created',
                value: AppFormatters.date(partnerTanker.createdAt),
              ),
              const SizedBox(height: AppSpacing.sm),
              _DetailRow(
                label: 'Updated',
                value: AppFormatters.date(partnerTanker.updatedAt),
              ),
            ],
          ),
        ),
        ...
        SecondaryButton(
          label: 'Delete Partner Tanker',
          icon: Icons.delete_outline_rounded,
          onPressed: () async {
            final confirmed = await MasterDialogs.confirmDelete(
              context,
              title: 'Delete Partner Tanker?',
              message:
                  'This will deactivate the partner tanker without removing history.',
            );
            if (!confirmed || !context.mounted) {
              return;
            }
            final result = await ref.read(deletePartnerTankerUseCaseProvider)(
              partnerTanker.id,
            );
            result.when(
              success: (_) {
                ref.invalidate(partnerTankerListProvider);
                ref.invalidate(selectedPartnerTankerProvider(partnerTanker.id));
                if (context.mounted) {
                  MasterDialogs.showSaved(context, 'Partner tanker deleted');
                  ref
                      .read(navigationServiceProvider)
                      .goToPartnerTankers(context);
                }
              },
              failure: (e) {
                if (context.mounted) {
