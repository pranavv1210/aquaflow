...
class WaterPointProfilePage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return switch (wp) {
      AsyncData<WaterPoint>(:final value) => _WaterPointProfileContent(
        waterPoint: value,
      ),
      AsyncError<WaterPoint>(:final error) => AppScreen(
        children: [
          const PageHeader(title: 'Water Point Profile', subtitle: 'Error'),
          ErrorStateWidget(
            title: 'Unable to load water point',
            message: error.toString(),
            onRetry:
                () => ref.invalidate(selectedWaterPointProvider(waterPointId)),
          ),
        ],
      ),
      _ => const AppScreen(
        children: [
          PageHeader(title: 'Water Point Profile', subtitle: 'Loading...'),
...
class _WaterPointProfileContent extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return AppScreen(
      children: [
        PageHeader(
          title: waterPoint.waterPointName,
          subtitle: 'Water Point Profile',
          trailing: IconButton.filledTonal(
            onPressed:
                () => ref
                    .read(navigationServiceProvider)
                    .goToEditWaterPoint(context, waterPoint.id),
        ...
        GlassCard(
          child: Column(
            children: [
              _DetailRow(
                label: 'Location ID',
                value: waterPoint.locationId ?? 'Not set',
              ),
              ...
              _DetailRow(
                label: 'Created',
                value: AppFormatters.date(waterPoint.createdAt),
              ),
              const SizedBox(height: AppSpacing.sm),
              _DetailRow(
                label: 'Updated',
                value: AppFormatters.date(waterPoint.updatedAt),
              ),
            ],
          ),
        ),
        ...
        SecondaryButton(
          ...
          onPressed: () async {
            final confirmed = await MasterDialogs.confirmDelete(
              ...
              message:
                  'This will deactivate the water point without removing history.',
            );
            if (!confirmed || !context.mounted) return;
            final result = await ref.read(deleteWaterPointUseCaseProvider)(
              waterPoint.id,
            );
            result.when(
              success: (_) {
                ref.invalidate(waterPointListProvider);
                ref.invalidate(selectedWaterPointProvider(waterPoint.id));
                if (context.mounted) {
                  MasterDialogs.showSaved(context, 'Water point deleted');
                  ref.read(navigationServiceProvider).goToWaterPoints(context);
                }
              },
              failure: (e) {
                if (context.mounted) {
