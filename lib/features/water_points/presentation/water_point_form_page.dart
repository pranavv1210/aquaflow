...
class _WaterPointFormPageState extends ConsumerState<WaterPointFormPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return switch (wp) {
      AsyncData<WaterPoint>(:final value) => () {
        _populateOnce(value);
        return _buildForm();
      }(),
      AsyncError<WaterPoint>(:final error) => AppScreen(
        children: [
          const PageHeader(title: 'Edit Water Point', subtitle: 'Error'),
          ErrorStateWidget(
            title: 'Unable to load water point',
            message: error.toString(),
            onRetry:
                () => ref.invalidate(
                  selectedWaterPointProvider(widget.waterPointId!),
                ),
          ),
        ],
      ),
      _ => const AppScreen(
        children: [
          PageHeader(title: 'Edit Water Point', subtitle: 'Loading...'),
          WaterPointListSkeleton(),
        ],
      ),
    };
  }
  ...
  Widget _buildForm() => BaseMasterForm(
    formKey: _formKey,
    title: _isEditing ? 'Edit Water Point' : 'Add Water Point',
    subtitle: _isEditing ? 'Update water point details' : 'Create water point',
    saveLabel: _isEditing ? 'Update Water Point' : 'Save Water Point',
    isSaving: _isSaving,
    onSave: _save,
    onCancel: _confirmCancel,
    children: [
      WaterPointFormFields(
  ...
  void _populateOnce(WaterPoint w) {
    if (_didPopulate) return;
    _nameController.text = w.waterPointName;
    _locationController.text = w.locationId ?? '';
  ...
  Future<void> _save() async {
    ...
    final input = WaterPointInput(
      waterPointName: _nameController.text,
      locationId: _locationController.text,
      notes: _notesController.text,
    );
    await saveWaterPointProviders(
      ref,
      waterPointId: widget.waterPointId,
      input: input,
      onSuccess: (WaterPoint saved) {
        if (!mounted) return;
        MasterDialogs.showSaved(
          context,
          _isEditing ? 'Water point updated' : 'Water point added',
        );
        _leaveForm(savedWaterPointId: saved.id);
      },
      onFailure: (msg) {
  ...
  void _leaveForm({String? savedWaterPointId}) {
    ...
    if (_isEditing) {
      nav.goToWaterPointProfile(
        context,
        savedWaterPointId ?? widget.waterPointId!,
      );
      return;
    }
