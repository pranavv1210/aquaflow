...
class _LocationFormPageState extends ConsumerState<LocationFormPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return location.when(
      loading: () => const AppScreen(children: [PageHeader(title: 'Edit Location', subtitle: 'Loading...'), LocationListSkeleton()]),
      error: (e, s) => AppScreen(children: [
        const PageHeader(title: 'Edit Location', subtitle: 'Error'),
        ErrorStateWidget(title: 'Unable to load location', message: e.toString(), onRetry: () => ref.invalidate(selectedLocationProvider(widget.locationId!))),
      ]),
      data: (Location loc) { _populateOnce(loc); return _buildForm(); },
    );
  }

  Widget _buildForm() => BaseMasterForm(
    formKey: _formKey, title: _isEditing ? 'Edit Location' : 'Add Location',
    subtitle: _isEditing ? 'Update location details' : 'Create location',
    saveLabel: _isEditing ? 'Update Location' : 'Save Location',
    isSaving: _isSaving, onSave: _save, onCancel: _confirmCancel,
    children: [LocationFormFields(nameController: _nameController, notesController: _notesController)],
  );

  Future<void> _confirmCancel() async {
    if (_hasChanges) { final discard = await MasterDialogs.confirmDiscard(context); if (!discard) return; }
    _leaveForm();
  }

  bool get _hasChanges => _nameController.text.trim().isNotEmpty || _notesController.text.trim().isNotEmpty;

  void _populateOnce(Location loc) { if (_didPopulate) return; _nameController.text = loc.locationName; _notesController.text = loc.notes ?? ''; _didPopulate = true; }

  Future<void> _save() async {
    if (!(_formKey.currentState?.validate() ?? false)) return;
    setState(() => _isSaving = true);
    final input = LocationInput(locationName: _nameController.text, notes: _notesController.text);
    await saveLocationProviders(ref, locationId: widget.locationId, input: input,
      onSuccess: (Location saved) { if (!mounted) return; MasterDialogs.showSaved(context, _isEditing ? 'Location updated' : 'Location added'); _leaveForm(savedLocationId: saved.id); },
  ...
  void _leaveForm({String? savedLocationId}) {
    ...
    if (_isEditing) { nav.goToLocationProfile(context, savedLocationId ?? widget.locationId!); return; }
